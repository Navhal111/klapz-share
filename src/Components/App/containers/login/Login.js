...
import NetworkModule from './../../components/network-module/index';
...
export default class Login extends Component {
  ...
  getOneTapLoginToken = () => {
    var url = klapzconfig.apiurl + 'temporary_tokens/one_tap_login/sessionId';
    console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
    ...
    .then((response) => {
      console.log("_________login token__________");
      if(response.errors){
        alert(response.errors.message);
      }else{
        this.setState({
          oneTapLoginToken: response.code
        })
      }
    })
    .catch(error => {
  ...
  login = async () => {
    ...
    // let url = klapzconfig.onetapLoginUrl + `initiate?clientId=${klapzconfig.onetapClientId}&correlationId=${this.state.oneTapLoginToken}&msisdn=${mobilenoWithCountry}&callBackUrl=${klapzconfig.apiurl}onetaplogin/callback`;
    // console.log(url);
    // console.log(NetworkModule);
    // try {
    //   let apiResponse =  await NetworkModule.get(url);
    ...
    .then((response) => {
      ...
      if(statusCode == 200) {
        ...
        this.props.OtpScreen({mobile: mobilenoWithCountry,country: this.state.selectedCountry.name})
      } else {
    ...
    return (
      <View style={styles.mainview}>
        ...
        <View style={styles.titleView}>
                <Text style={styles.textTitle}>What's your Mobile</Text>
          </View>
          <View style={styles.titleView}>
          ...
          <View style={styles.titleView}>
                <Text style={styles.textSubTitle}>on this number, by SMS.</Text>
          </View>
          ...
          <View style={styles.inputFieldSec}>
            ...
            <View style={styles.signInSec}>
              ...
              >
                <Text style={styles.textOrange}>Next</Text>
              </TouchableOpacity>
            </View>
            ...
            >
              <View style={[styles.container,{paddingTop: Platform.OS=="ios"?40:20,paddingHorizontal: 20}]}>
                  ...
                  <View style={{flexDirection: "row",justifyContent: "space-between"}}>
                      ...
                      <View style={styles.contactHeadingWrapper}>
                          <Text style={{fontSize: 16,color: Colors.background,fontFamily: "Montserrat-Regular"}}>Choose country</Text>
                      </View>
                  ...
                  <View style={[styles.flatListContainer,{paddingVertical: 10}]}>
                      <TextInput
                          ...
                          style={{borderRadius: 7,borderWidth: 1,borderColor: "lightgrey",height: 40,paddingHorizontal: 10,fontFamily: "Montserrat-Regular"}}
                          onChangeText={(text) => {
                          ...
                          renderItem={({item,index}) => {
                              return (
                                  <TouchableOpacity onPress={() => {
                                      ...
                                      <Text style={{fontSize: 16,marginLeft: 10,fontFamily: "Montserrat-Regular"}}>{item.name}</Text>
                                  </TouchableOpacity>
                              )
                          }}
